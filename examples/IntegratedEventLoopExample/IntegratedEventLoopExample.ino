...
static void serial_write_line(LineView line) {
...
struct ServerLineHandler final : public ITcpLineServerHandler {
...
struct ClientLineHandler final : public ITcpLineClientHandler {
...
TcpLineServerHandler<160> server_handler(server_line_handler);
ClientLineHandler client_line_handler;
TcpLineClientHandler<160> client_handler(client_line_handler);
...
void setup() {
    ...
    if (wifi_result != WiFiConnectResult::Connected) {
        serial_write_text(wifi_connect_result_text(wifi_result));
        serial_write_text("\n");
