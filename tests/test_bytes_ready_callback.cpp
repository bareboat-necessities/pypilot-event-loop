...
int main() {
    ...
    async_event_loop::LinuxFdStream reader(fds[0]);
    async_event_loop::LinuxFdStream writer(fds[1]);

    int bytes_read = 0;
    const async_event_loop::EventHandle handle = event_loop.on_bytes_ready(reader, [&]() {
        uint8_t buf[8];
