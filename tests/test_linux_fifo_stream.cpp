...
int main() {
    ...
    async_event_loop::LinuxFifoByteStream fifo(path);
    assert(fifo.valid());
    assert(fifo.native_fd() >= 0);

    int lines = 0;
    async_event_loop::LineProtocolReader<64> reader(
        fifo,
        async_event_loop::LineProtocolOptions{},
        [&](async_event_loop::LineView line) {
            (void)line;
            ++lines;
        });

    const async_event_loop::EventHandle handle = event_loop.on_bytes_ready(fifo, [&]() {
